...
router.post('/vendedores', (req, res) => {
    const { nome, email } = req.body;
    db.run('INSERT INTO vendedores (nome, email) VALUES (?, ?)', [nome, email], function (err) {
        if (err) return res.status(500).json({ error: err.message });
        res.json({ id: this.lastID, nome, email });
        console.log(`${nome}, ${email}`)
    });
...
router.get('/vendedores', (req, res) => {
    const sql = `
        SELECT v.id, v.nome, v.email, p.id AS produto_id, p.nome AS produto_nome, p.preco
        FROM vendedores v
...
router.put('/vendedores/:id', (req, res) => {
    ...
    // Pr aatualizar só os campos vazios
    ...
    db.run(query, [nome, email, id], function (err) {
        if (err) return res.status(500).json({ error: err.message });
        res.json({ id, nome, email, message: 'Vendedor atualizado com sucesso' });
        console.log(`Vendedor ${id} atualizado`);
    });
});
...
router.get('/vendedores/:id/produtos', (req, res) => {
    const { id } = req.params;
    db.all(
        `SELECT p.id, p.nome, p.preco FROM produtos p
